...
CidadesAL <- CidadesAL%>% 
  ...
  mutate(prim.ano = min(ano), #Separa o primeiro ano que o município tem um registro
         ...
         Municipio_UF = paste(City, UF, sep = "_")) %>% #Cria coluna com municipio e UF, para facilitar junção às outras bases
...
# 1.5 Adicionando dias desde o último desastre.
...
ALClima <- readRDS(file = here("Dados/ALClima.RDS"))


Desastres <- readRDS(file = here("Dados/Desastres.RDS"))%>% 
