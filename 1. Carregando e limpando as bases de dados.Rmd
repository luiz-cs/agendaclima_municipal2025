...
ndes_ano <- Desastres %>%
  ...
  geom_col() +
  labs(
    title = "Número de desastres no Brasil, por categoria e ano",
    y = "Número de desastres",
    x = "Ano",
    fill = "Categorias de Desastres",
    caption = "Fonte dos dados: Atlas Digital de Desastres no Brasil. Elaboração do Autor."
  ) +
  theme_linedraw() +
...
ndes_ano
ggsave(here("Figuras/1.2Desastres_Anos_Brasil.png"), ndes_ano, width = 10, height = 5)
  
rm(ndes_ano, paleta_desastres)
```

```{r, 'salvando_desastres'}
...
sapl_mun_years <- df_contagem %>% 
  ...
  theme_linedraw()+
  labs(title = "Número de municípios na base de dados por ano x Total de proposituras nos anos subsequentes", 
       y = "Número de municípios", 
       x = "Ano", 
       caption = "Dados levantados através dos sites das Câmaras Municipais que utilizam o SAPL. Elaboração do autor.")+
  theme(title = element_text(size = 9),
        text = element_text(family = "Times New Roman"),
        axis.title = element_text(size = 14),
        axis.text = element_text(size = 14),
        legend.text = element_text(size = 10))   # Cor do título do eixo Y à esquerda

sapl_mun_years

ggsave(here("Figuras/1.4SAPL_recorte_temporal.png"), sapl_mun_years, width = 10, height = 5)
...
Agora, faremos mapas dos indicadores de Risco Climático

```{r, 'mapas_risco_climatico', warning=FALSE}
#Pacotes para mapas
library(sf)
library(geobr)
...
#Cria uma base reduzida para análise com mapa:
mapaAB <- AdaptaBrasil %>% 
  select(Cod_IBGE_Mun, starts_with("AB2_")) %>% 
  pivot_longer(cols = starts_with("AB2_"), names_prefix = "AB2_", names_to = "Categoria", values_to = "Índice de Risco") %>% 
mutate(
  Categoria = sub("^([^_]+)_([^_]+)_", "\\1_\\2: ", Categoria),
  Categoria = gsub("_", " ", Categoria)) %>% 
  rename("code_muni" = Cod_IBGE_Mun)

mapaAB <- merge(mapa_brasil, mapaAB, by = "code_muni", all.x = TRUE) %>% 
  rename("geom" = geometry) %>% 
  mutate(Categoria = gsub("Desastres Hidrológicos: ", "", Categoria))

mapa_ab <- mapaAB %>% 
  filter(!is.na(`Índice de Risco`)) %>% 
  ggplot(aes(fill= `Índice de Risco`)) +
  geom_sf(color = NA)+
  labs(title="Índice de Risco Climático - Adapta Brasil",
       caption='Fonte: MCTI/Adapta Brasil. Elaboração própria', size=8)+
  scale_fill_gradient2(low = "darkgreen", mid = "yellow", high = "darkred", midpoint = 0.5, name = "Índice de Risco")+
  theme_linedraw()+
  theme(
...
ggsave(here("Figuras/1.7.1 Mapa_AdaptaBrasil.png"), mapa_ab)

rm(mapa_brasil, mapaAB, mapa_ab)
```
...
df_grid <- CJ( #Cria grid de datas
  Data = unique(CidadesAL$Data),
  Municipio_UF = unique(CidadesAL$Municipio_UF)
)

df_ols <- merge(df_grid, df_ols, by = c("Data", "Municipio_UF"), all = T)  
...
saveRDS(total_prop, here("Dados/total_proposituras.RDS"))

rm(CidadesAL, total_prop, df_grid)
```
...
#Adicionando Receita per Capita:

df[, Receitas_pCap := (Receitas_Mun/Populacao)]
df_ols[, Receitas_pCap := (Receitas_Mun/Populacao)]
```
...
df<- df %>% 
  ...
  select(Data, Ano, Mes, Dia, 
         ...
         Receitas_Mun, Receitas_pCap, Populacao, proxima_eleicao, dias_proxima_eleicao, n_OSCs, 
...
df_ols <- df_ols %>% 
  select(Data, Ano, Mes, Dia, Cod_IBGE_Mun, Municipio_UF,
         ...
         Desastre, Tipo_Desastre, Obitos, eleicao, Receitas_Mun, Receitas_pCap, n_OSCs, Populacao, 
