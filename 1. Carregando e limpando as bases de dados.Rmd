...
# Merge df with Desastres to get all combinations by Cod_IBGE_Mun
df_prev <- merge(df, Desastres, by = "Cod_IBGE_Mun", allow.cartesian = TRUE, suffixes = c("", "_desastre"))
# Filter where disaster happened before project
df_prev <- df_prev[Data_desastre < Data]
# Keep the latest disaster per (UF, Municipio_UF, Cod_IBGE_Mun, Nome_Municipio, Data)
...
# Merge df with df_prev and df_next
...
df <- merge(df, df_prev, by = c("UF", "Municipio_UF", "Cod_IBGE_Mun", "Data"), all.x = TRUE)
df <- merge(df, df_next, by = c("UF", "Municipio_UF", "Cod_IBGE_Mun", "Data"), all.x = TRUE)
...
df_prev <- df_ols %>%
  ...
  filter(Data_desastre < Data) %>% 
  group_by(UF, Municipio_UF, Cod_IBGE_Mun, Nome_Municipio, Data) %>%   
  slice_max(Data_desastre, with_ties = FALSE) %>% 
  ungroup() %>%
  select(UF, Municipio_UF, Cod_IBGE_Mun, Nome_Municipio, Data, data_desastre_anterior = Data_desastre, desastre_anterior = Desastre, obitos_desastre_anterior = DH_MORTOS) 
...
#Junta os arquivos na base original:
df_ols <- df_ols %>%
  left_join(df_prev, by = c("Municipio_UF", "Cod_IBGE_Mun", "Data")) %>%
  left_join(df_next, by = c("Municipio_UF", "Cod_IBGE_Mun", "Data")) %>%
  mutate(
    dias_desastre_anterior = as.integer(Data - data_desastre_anterior),
    dias_desastre_proximo = as.integer(Data - data_desastre_proximo)
  )
