...
### ALClima e Criando Grid de Datas:

Para criar uma base com a linha do tempo completa, iremos começar criando um grid de datas
...
df <- ALClima13_23 %>% 
  select(Cod_IBGE_Mun, Municipio_UF, Nome_Municipio, Tipo_Projeto, UF, Tipo, prim.data, prim.ano) %>% 
  unique()

# Add  ID:
df[, temp_id := .I]

# Cross join usando CJ (fast)
df <- CJ(temp_id = df$temp_id, Data = seq(as.Date("2013-01-01"), as.Date("2023-12-31"), by = "day"))[
  df, on = .(temp_id), nomatch = 0
]
...
df <- merge(df, ALClima13_23, by = c("Data", "Municipio_UF"), all = T)
```
...
df <- df %>%
  mutate(Cod_IBGE_Mun = ifelse(is.na(Cod_IBGE_Mun.x), Cod_IBGE_Mun.y, Cod_IBGE_Mun.x)) %>% 
  select(!c("Cod_IBGE_Mun.x", "Cod_IBGE_Mun.y"))
...
Desastres <- readRDS(here("Dados/Desastres13_23.RDS"))
...
rm(ALClima_prev, ALClima_next, lista_municipios, Desastres)
