...
OSCsClima <- OSCs %>% 
  group_by(Cod_IBGE_Mun, municipio_nome, sigla_uf, dt_fundacao_osc) %>% 
  summarise(n_OSCs = n())
...
```{r}
#Adicionando número de OSCs na data, até o respectivo dia

#Criando grid de datas
date_grid <- expand_grid(
  Cod_IBGE_Mun = unique(OSCsClima$Cod_IBGE_Mun),
  Date = seq(as.Date("1991-01-01"), as.Date("2023-12-31"), by = "day")
)

#PAREI AQUI
n_OSCsClima <- date_grid %>%
  left_join(OSCsClima, by = "Cod_IBGE_Mun") %>%
  filter(dt_fundacao_osc <= Date) %>%
  group_by(Cod_IBGE_Mun, Date) %>%
  summarise(n = sum(n_OSCs, na.rm = TRUE), .groups = "drop")

saveRDS(OSCsClima, here("Dados/n_OSCsClima.RDS"))

rm(OSCs, OSCsClima, lista_municipios)
```
...
Para facilitar a leitura, o Senado Federal, através do "Panorama do Legislativo Municipal", compila os dados dos orçamentos municipais já tratados.

Dados: <https://www.senado.leg.br/institucional/datasenado/panorama/#/dados>

```{r}
orcamento <- read.csv2(here("Dados/Panorama do Legislativo Municipal/orcamentodomunicipio.csv"), locale = locale(encoding = "Latin1"))
```
