...
ggsave(here("Figuras/2.3.1.3P_Categoria_Desastres_Ano.png"), p_des_ano_cat, width = 10, height = 15)
...
ggsave(here("Figuras/2.3.1.4P_Categoria_Desastres_Obitos.png"), p_des_ano_obitos, width = 10, height = 15)
...
osc_ano <- OSCs %>%
  ...
  geom_point(size = 3)+
  labs(
    title = "Número médio de OSCs, por ano",
...
orcamento_dist <- orcamento %>%
  ...
  geom_density()+
  labs(
    title = "Distribuição da Receita dos municípios, em 2023",
    y = "Frequência",
    x = "Média",
    fill = "Origem"
  ) +
...
ggsave(here("Figuras/2.3.3.2.Distribuição de Receitas em 2023.png"), orcamento_dist)
...
prop_desastres <- df %>% 
  ...
  group_by(dias_desastre_anterior, Cod_IBGE_Mun) %>% 
  summarise(n = n(), 
            proj_dia = sum(total_tipo_proj_dia)) %>% 
  group_by(dias_desastre_anterior) %>% 
  summarise(n = mean(n), 
            proj_dia = mean(proj_dia),
            Percentual = n/proj_dia) %>% 
  data.table() %>% 
  ggplot(aes(x = dias_desastre_anterior, y = n)) +
  geom_line()+
  xlim(0,30)+
  ylim(0,15)+
  theme_bw()
...
df %>% 
  filter(!is.na(dias_desastre_anterior), 
         !is.na(dias_desastre_proximo)) %>%
  pivot_longer(starts_with("dias_desastre_"), names_prefix = "dias_desastre_", names_to = "Relacao_Desastre", values_to = "Dias_desastre") %>% 
  group_by(Dias_desastre) %>% 
  summarise(n = n()) %>% 
  data.table() %>% 
  ggplot(aes(x = Dias_desastre, y = n)) +
  geom_line()+
...
df %>% 
  ...
  geom_histogram(binwidth = 30)+ #Cada um representa 3 meses
  theme_bw()+
  xlim(0,730)
```
...
```{r}
df %>% 
  group_by(dias_desastre_anterior, Cod_IBGE_Mun, desastre_proximo) %>% 
  summarise(n = n(), 
            proj_dia = sum(total_tipo_proj_dia)) %>% 
  group_by(dias_desastre_anterior, desastre_proximo) %>% 
  summarise(n = mean(n), 
            proj_dia = mean(proj_dia),
            Percentual = n/proj_dia) %>% 
  data.table() %>% 
  ggplot(aes(x = dias_desastre_anterior, y = Percentual)) +
  geom_line()+
  xlim(0,365)+
  theme_bw()+
  facet_wrap(~ desastre_proximo)
```
...
df %>% 
  ...
  summarise(n = n()) %>% 
  data.table() %>% 
  ggplot(aes(x = Dias_desastre, fill = T)) +
  geom_density()+
  geom_vline(xintercept = 0, linetype = "dashed")+
  theme_bw()+
  theme(legend.position = "none")
...
df %>% 
  filter(!is.na(dias_desastre_anterior), 
         !is.na(dias_desastre_proximo)) %>%
  pivot_longer(starts_with("dias_desastre_"), names_prefix = "dias_desastre_", names_to = "Relacao_Desastre", values_to = "Dias_desastre") %>% 
  group_by(Dias_desastre, desastre_anterior) %>% 
  summarise(n = n()) %>% 
  data.table() %>% 
  ggplot(aes(x = Dias_desastre, fill = desastre_anterior)) +
  geom_density()+
  theme_bw()+
  theme(legend.position = "none")+
...
```{r}
df %>% 
  ggplot(aes(x = dias_desastre_proximo, fill = T)) +
  geom_density()+
  theme_bw()+
  theme(legend.position = "none")+
  xlim(-730, 0)
