...
regiao_percent <- regiao %>%
  ...
  scale_y_continuous(labels = label_percent(accuracy = 1)) +
  labs(
    title = "Percentual de municípios presentes na seleção, por Região",
    x = "Regiões",
    y = "Percentual de Municípios",
    fill = "Regiões", 
       caption='Fonte: Elaboração do autor', size= 10 
...
pop_hist <- Populacao %>%
  ...
  geom_histogram(color = "black", fill= "blue", alpha = 0.5, binwidth = 10000) +
  labs(
    title = "Distribuição da população dos municípios, em 2024",
    y = "Número de municípios",
    x = "Faixas de População",
       caption="Bins = 10 mil habitantes. Elaboração do autor", size= 10) +
  theme_bw() +
  theme_classic() +
  theme(legend.position = "bottom",
        text = element_text(family = "Times New Roman"),
...
ggsave(here("Figuras/2.1.4.Histograma_População.png"), pop_hist, width = 10, height = 5)
...
tiproj <- merge(tiproj, CidadesAL, by = c("Ano", "Tipo"), all.x = T) %>% 
  mutate(p_proj_clima_ano = n_proj_clima/total_proj_tipo)
...
ptipano
ggsave(here("Figuras/2.2.4.1P_Tipos_Prop_Ano.png"), ptipano, width = 10, height = 8)

rm(ptipano, CidadesAL)
```
...
tiproj <- tiproj %>% 
  ...
  summarise(n_proj_clima = sum(n_proj_clima),
            total_proj_tipo = sum(total_proj_tipo), 
            p_proj_clima_ano = n_proj_clima/total_proj_tipo)

#Gráfico
ppano <- tiproj %>%
  ggplot(aes(x = Ano, y = p_proj_clima_ano)) + 
  geom_line(aes(size = 1)) +
  geom_point(aes(size = 2))+
  theme_linedraw() +
  labs(
    title = "Proporção dos tipos de proposituras climáticas em relação ao total de projetos, por ano", 
    y = "Percentual de proposituras", 
    x = "Ano", 
       caption='Fonte: Elaboração do autor', size= 10
  ) +
  scale_y_continuous(labels = scales::label_percent(accuracy = 0.1), limits = c(0, 0.015)) +
  scale_x_continuous(labels = scales::label_number(accuracy = 1), n.breaks = 8) +
  scale_fill_manual(
    values = paleta_proposituras$Cores,
    limits = paleta_proposituras$Tipo) +
  theme(
...
ggsave(here("Figuras/2.2.4.2P_Tipos_Prop_Ano.png"), ppano, width = 10, height = 5)
...
ggsave(here("Figuras/2.2.6N_climatermos_ano.png"), width = 10, height = 8, ct_nano)
...
### 2.2.9 Cruzamento da frequência dos termos.

```{r, warning= FALSE}
# Criando base só com clima_termos

climatermos2 <- df %>% 
  select(clima_termos)

#Adiciona coluna row_id
climatermos2 <- climatermos2[
  , paste0("_clima", 1:6) := tstrsplit(clima_termos, ",")
][
  , row_id := .I, 
]

#Retornando para uma coluna
climatermos2 <- melt(climatermos2,
                   id.vars = "row_id",
                   measure.vars = patterns("^_clima"),
                   value.name = "clima_termo",
                   na.rm = TRUE
)[
  , clima_termo := trimws(clima_termo)]

#Contando totais:
totals <- climatermos2[, .(term1 = clima_termo)][
  , .(total = .N), by = term1]
...
climatermos2 <- climatermos2[
  , {
    termos <- unique(clima_termo)
    if (length(termos) == 1L) {
      .(term1 = termos, term2 = termos)  # self-pair
    } else {
      CJ(term1 = termos, term2 = termos)[term1 != term2]
    }
  }, by = row_id
]
...
climatermos2 <- merge(climatermos2, totals, by = "term1")[
  , pct := N / total
]

# Criando matriz para plotagem:
all_terms <- sort(unique(c(climatermos2$term1, climatermos2$term2)))

all_terms <- CJ(term1 = all_terms, term2 = all_terms)

climatermos2 <- merge(all_terms, climatermos2[, .(term1, term2, pct)], by = c("term1", "term2"), all.x = TRUE)

climatermos2[is.na(pct), pct := 0]


#Gráfico:
p_cooctermos <- ggplot(climatermos2, aes(x = term1, y = term2, fill = pct)) +
  geom_tile(color = "white") +
   geom_text(aes(label = scales::percent(pct, accuracy = 1)), size = 3)+
  scale_fill_gradient(low = "white", high = "darkred", labels = scales::label_percent(accuracy = 1)) +
  labs(
    title = "Taxa de coocorrência entre termos climáticos",
...
ggsave(here("Figuras/2.2.9P_Coocorrencia_Termos_Climáticos.png"), p_cooctermos, height = 10, width = 10)

rm(all_terms, climatermos2, totals, p_cooctermos )
```

### 2.2.10 Distribuição do número de proposituras, por município
...
mun_n_clima <- df_mun_n %>% 
  ...
  labs(
    title = "Boxplot do número de proposituras climáticas, por municípios",
    y = "",
...
ggsave(here("Figuras/2.2.10Distribuição_N_Municipios.png"), mun_n_clima, height = 3, width = 10)

rm(df_mun_n, mun_n_sum, mun_n_clima)
```
...
df_mun_p<- df_ols %>% 
  ...
  summarise(n = sum(n_proj_clima),
            total = sum(total_proj_dia),
            agenda_clima = n/total) %>% 
  merge(df_mun_n, by = "Cod_IBGE_Mun", all = T)

mun_p_sum <- summary(df_mun_p$agenda_clima)
mun_p_sum  

mun_p_clima <- df_mun_p %>% 
  ggplot(aes(x = agenda_clima))+
  geom_boxplot(fill = "darkgreen")+ 
  labs(
    title = "Boxplot do percentual de proposituras climáticas, por municípios",
    y = "",
    x = "Percentual de proposituras",
    fill = "Origem", 
       caption='Fonte: Elaboração do autor') +
...
ggsave(here("Figuras/2.2.11Distribuição_P_Municipios.png"), mun_p_clima, height = 3, width = 10)

rm(df_mun_p, mun_p_sum, mun_p_clima)
```
...
Sazonalidade_Todos <- readRDS(here("Dados/CidadesAL13_24.RDS")) %>% 
...
sazonalidade <- merge(sazonalidade, Sazonalidade_Todos, by = c("Ano_Mes"), all.x = T) %>% 
  mutate(p_total = n/total_proj)

rm(Sazonalidade_Todos)
```

### 2.3.1 Série Temporal de proposituras por mês - Volume absoluto

```{r warning = False}


# Passo 1: gerar posições (índices) e labels
n_ticks <- nrow(sazonalidade)
x_vals <- sazonalidade$Ano_Mes

# Criar vetores com as posições desejadas
vlines_idx <- seq(12.5, n_ticks, by = 12)  # linhas verticais após cada ano (após mês 12)
label_idx  <- seq(1, n_ticks, by = 4)    # mostrar só 1 de cada 4 rótulos

# Gráfico com ajustes
sazon1 <- sazonalidade %>%
  ggplot(aes(y = n, x = factor(Ano_Mes, levels = unique(Ano_Mes)))) +  
  geom_line(group = 1) +
  geom_point(size = 2)+
  geom_vline(xintercept = vlines_idx, linetype = "dashed") +
  scale_x_discrete(breaks = x_vals[label_idx]) +
  labs(
    title = "Número de proposituras climáticas, por mês",
    x = "Ano e Mês",
...
ggsave(here("Figuras/2.3.1Série_Temporal_Proposituras.png"), sazon1, height = 5, width = 10)

rm(n_ticks, x_vals, vlines_idx, label_idx, sazon1)
```

### 2.3.2 Série Temporal de proposituras por mês - Volume relativo
...
# Gráfico com
sazon2 <- sazonalidade %>%
  ggplot(aes(y = p_total, x = factor(Ano_Mes, levels = unique(Ano_Mes)))) + 
  geom_line(group = 1) +
  geom_point(size = 2)+
  geom_vline(xintercept = vlines_idx, linetype = "dashed") +
  scale_x_discrete(breaks = x_vals[label_idx]) +
  labs(
    title = "Percentual de proposituras climáticas em relação ao total, por mês",
    x = "Ano e Mês",
    y = "Percentual do total de proposituras",
    caption = "Fonte: Elaboração do autor", size = 10
  ) +
  theme_bw() +
  scale_fill_manual(values = paleta_proposituras$Cores, limits = paleta_proposituras$Tipo) +
...
ggsave(here("Figuras/2.3.2PSerie_Temporal_Proposituras.png"), sazon2, height = 5, width = 10)

rm(n_ticks, x_vals, vlines_idx, label_idx, sazon2)
```

### 2.3.3. Percentual em relação ao total de proposituras, por mês

```{r, warning= FALSE}
psazonalidade <- sazonalidade %>% 
  group_by(Mes) %>% 
  summarise(n = sum(n),
            total_proj = sum(total_proj), 
            p_total = n/total_proj) %>% 
  ggplot(aes(x = Mes, y = p_total)) +
...
ggsave(here("Figuras/2.3.3Sazonalidade_Proposituras.png"), psazonalidade, width = 10, height = 5)

rm(psazonalidade, sazonalidade)
```

## 2.4Variáveis explicativas:
...
```{r, anos_desastres, warning = FALSE}
...
ggsave(here("Figuras/2.4.1.1N_Desastres_Anos.png"), ndes_ano, width = 10, height = 5)
...
```{r, 'letalidade_desastres', warning = FALSE}
...
ggsave(here("Figuras/2.4.1.2N_Des_Letalidade_Anos.png"), nlet_ano, width = 10, height = 5)
...
obitos <- Desastres %>% 
  filter(Cod_IBGE_Mun %in% lista_municipios$Cod_IBGE_Mun, 
         DH_MORTOS > 0, 
         Ano >= 2013) %>% 
    ggplot(aes(y = DH_MORTOS, fill = Desastre)) +
  geom_boxplot() +
  labs(
    title = "Boxplot de óbitos por desastres registrados nos municípios selecionados (2013-2024)",
    y = "Número de Mortos",
    x = "Tipos de Desastres",
    fill = "Categorias de Desastres",
    caption = "Elaborado pelo autor") +
  theme_classic() +
  scale_fill_manual(values = paleta_desastres$Cores, limits = paleta_desastres$Tipo)+
  ...
  facet_grid(cols = vars(Desastre))
  
obitos

ggsave(here("Figuras/2.4.1.3Boxplot_Des_Letalidade.png"), obitos, width = 10, height = 4)
...
dfdesastres <- dfdesastres %>% 
  rename("n_Selecionados" = n, 
         "obitos_Selecionados" = DH_MORTOS) %>% 
  group_by(Ano) %>% 
  mutate(p_Selecionados = n_Selecionados/sum(n_Selecionados), 
         pc_obitos_Selecionados = obitos_Selecionados/sum(obitos_Selecionados)) %>% 
...
```{r, 'graf_comparacao_brasil', warning = FALSE}
...
ggsave(here("Figuras/2.4.1.4P_Categoria_Desastres_Ano.png"), p_des_ano_cat, width = 8, height = 10)

rm(totalDes, p_des_ano_cat)
```
...
ggsave(here("Figuras/2.4.1.5P_Categoria_Desastres_Obitos.png"), p_des_ano_obitos, width = 8, height = 10)

rm(p_des_ano_obitos, dfdesastres)
```

```{r, warning=FALSE}
#Sazonalidade dos Desastres
mes_todos_des <- Desastres %>%
  mutate(Mes = month(as.Date(Data_Evento))) %>% 
  group_by(Desastre, Mes) %>% 
  summarise(n_Brasil = n()) %>% 
  group_by(Mes) %>% 
  mutate(p_Brasil = n_Brasil/sum(n_Brasil))

#Novo Desastres  
sazon_desastres <- Desastres %>%
  filter(Cod_IBGE_Mun %in% lista_municipios$Cod_IBGE_Mun) %>%  
  mutate(Mes = month(as.Date(Data_Evento))) %>%
  group_by(Desastre, Mes) %>% 
  summarise(n_Selecionados = n()) %>% 
  group_by(Mes) %>% 
  mutate(p_Selecionados = n_Selecionados/sum(n_Selecionados)) %>% 
    merge(mes_todos_des, by = c("Desastre", "Mes"), all= T) %>% 
  replace(is.na(.), 0) %>% 
    pivot_longer(cols = starts_with("n_"), names_prefix = "n_", names_to = "origem", values_to = "n") %>% 
  ggplot(aes(x = Mes, y = n, fill = Desastre))+
  geom_col(stat = "identity")+
  labs(
    title = "Número de desastres na seleção, por Mês",
    y = "Número de desastres",
    x = "Ano",
    fill = "Categorias de Desastres",
    caption = "Elaborado pelo autor") +
  ...
  scale_x_continuous(n.breaks = 10)+
    theme(legend.position = "bottom",
        ...
        legend.text = element_text(size = 10),
        strip.text = element_text(size = 14))+
    facet_wrap(~origem, scales = "free_y", ncol = 1)

sazon_desastres

ggsave(here("Figuras/2.4.1.6Sazonalidade_Desastres.png"), sazon_desastres, width = 10, height = 8)

rm(sazon_desastres, mes_todos_des)
```

### 2.4.1.7 - Número médio de desastres por ano

```{r, warning = FA}
df_ols <- readRDS(here("Dados/df_ols.RDS"))
#Número médio de desastres por dia, por munípio
media_desastres <- df_ols %>% 
  group_by(Cod_IBGE_Mun, Ano) %>% 
  summarise(n_desastres = sum(Desastre))

media_desastres <- summary(media_desastres$n_desastres)
media_desastres
#Base com todos a média de todos os desastres
des_mun_todos <- df_ols %>%
  filter(Cod_IBGE_Mun %in% lista_municipios$Cod_IBGE_Mun) %>% 
  group_by(Cod_IBGE_Mun, Ano) %>% 
  summarise(n = sum(Desastre)) %>% 
  group_by(Cod_IBGE_Mun) %>% 
  summarise(`Desastres por Ano` = mean(n)) %>% 
  mutate(Origem = "Selecionados")

grid_Desastres <- expand.grid(Cod_IBGE_Mun = lista_municipios$Cod_IBGE_Mun,
                              Ano = c(2013:2024))

#Gráfico
dfdesastres_mun <- Desastres %>%
  group_by(Cod_IBGE_Mun, Ano) %>% 
  summarise(n = n()) %>% 
  merge(grid_Desastres, all.y = T) %>% 
  mutate(n = ifelse(is.na(n), 0, n)) %>% 
  group_by(Cod_IBGE_Mun) %>% 
  summarise(`Desastres por Ano` = mean(n)) %>% 
  mutate(Origem = "Brasil") %>% 
  bind_rows(des_mun_todos) %>% 
  ggplot(aes(x = `Desastres por Ano`, fill = Origem)) +
  geom_density() +
  geom_vline(xintercept = media_desastres[4], linetype = "dashed")+
  labs(
    title = "Distribuição da Média de Desastres por Município, por ano",
    y = "Frequência Relativa",
    x = "Média de Desastres por Ano (em log)",
    caption = "Fonte: Atlas Digital de Desastres no Brasil. Gráfico elaborado pelo autor", size = 12) +
...
ggsave(here("Figuras/2.4.1.7Desastres_por_Ano.png"), dfdesastres_mun, width = 10, height = 5)

rm(media_desastres, des_mun_todos, grid_Desastres, dfdesastres_mun)
```

## 2.4.2 Riscos Climáticos
...
AdaptaBrasil <- readRDS(here("Dados/AdaptaBrasil.RDS")) %>% 
  ...
  filter(!is.na(Indice_Risco), 
         Dimensao_Risco != "Recursos Hídricos") %>% 
  data.table()
...
#Tabela
sumABSelecionados <- AdaptaBrasil %>% 
  filter(Nivel == 2, 
         origem == "Municipios Selecionados") %>% 
  group_by(Dimensao_Risco, Risco_Climatico) %>% 
...
tb1 <- AdaptaBrasil %>% 
  ...
  merge(sumABSelecionados, by = c("Dimensao_Risco", "Risco_Climatico")) %>% 
  ...
  select(Dimensao_Risco, Risco_Climatico, Media_selecionados, Media_Brasil, Diferenca_Media, Mediana_selecionados, Mediana_Brasil, Diferenca_Mediana, DesvPad_selecionados, DesvPad_Brasil, Diferenca_DesvPad)
...
dist_AB <- AdaptaBrasil %>%
  ...
  filter(Nivel == 2) %>% 
  mutate(Risco_Climatico = ifelse(Risco_Climatico == "Inundações enxurradas e alagamentos", "Inundações e afins", Risco_Climatico)) %>% 
  ...
  labs(
    ...
    x = "Nível de Risco",
    fill = "Categorias de Desastres",
    caption = "Fonte: AdaptaBrasil. Gráfico elaborado pelo autor", size = 12) +
  theme_linedraw()+
    theme(legend.position = "none",
        text = element_text(family = "Times New Roman"),
        axis.title = element_text(size = 14),
        ...
        legend.text = element_text(size = 10),
        strip.text = element_text(size = 12))+
    facet_grid(origem ~ Risco_Climatico)

dist_AB
ggsave(here("Figuras/2.4.2.1Distribuicao_Risco_Climatico.png"), dist_AB, width = 10, height = 5)
  
rm(dist_AB, sumABSelecionados, tb1)
```
...
tb2 <- AdaptaBrasil %>% 
  ...
  mutate(Diferenca_Media = Media_selecionados - Media_Brasil, 
         Diferenca_DesvPad = DesvPad_selecionados - DesvPad_Brasil) %>% 
  select(Dimensao_Risco, Risco_Climatico, Media_selecionados, Media_Brasil, Diferenca_Media, DesvPad_selecionados, DesvPad_Brasil, Diferenca_DesvPad)
...
dist_AB_componentes <- AdaptaBrasil %>%
  ...
  labs(
    ...
    x = "Nível de Risco",
    fill = "Origem",
    caption = "Elaborado para o autor") +
  theme_linedraw()+
  theme(legend.position = "bottom")+
  scale_y_continuous(labels = label_percent(accuracy = 1)) +
      theme(legend.position = "bottom",
...
ggsave(here("Figuras/2.4.2.2Distribuicao_Componentes_Climaticos.png"), dist_AB_componentes, width = 10, height = 15)
  
rm(dist_AB_componentes, sumAB, sumABSelecionados, AdaptaBrasil, tb2)
```

## 2.4.3 Presença de OCSs nos Municípios
...
osc_ano <- OSCs %>%
  ...
  labs(
    ...
    x = "Ano",
    fill = "Origem",
    caption = "Fonte: Mapa das OSCs/IPEA. Gráfico elaborado pelo autor") +
  ylim(0,200)+
  scale_x_continuous(n.breaks = 10)+
  theme_bw()+      
  theme(legend.position = "bottom",
...
ggsave(here("Figuras/2.4.3.1.Média de OSCs por município e ano.png"), osc_ano, width = 10, height = 4)
...
ggsave(here("Figuras/2.4.3.2.Distribuição de OSCs em 2023.png"), osc_dist, width = 8, height = 3)
  
rm(osc_dist, OSCs)
```

## 2.4.4 Orçamento dos municípios
...
orcamento_ano <- orcamento %>%
  ...
  labs(
    ...
    x = "Ano",
    fill = "Origem",
    caption = "Fonte: SICONFI. Gráfico produzido pelo autor.") +
  scale_x_continuous(n.breaks = 11)+
  scale_y_continuous(labels = scales::comma, limits = c(0, 600000000))+
  theme_bw()+
  theme(legend.position = "bottom",
        text = element_text(family = "Times New Roman"),
        axis.title = element_text(size = 14),
...
ggsave(here("Figuras/2.4.4.1.Média de Receitas por ano.png"), orcamento_ano, width = 10, height = 4)
  
rm(orcamento_ano)
  
```

### Distribuição do orçamento dos municípios: selecionados vs Brasil
...
ggsave(here("Figuras/2.4.4.2.Distribuição de Receitas em 2024.png"), orcamento_dist, width = 8, height = 3)
  
rm(orcamento_dist, orcamento)
```

# 2.5 Cruzando as variáveis
...
## 2.5.1 Proposituras x Desastres

### 2.5.1.1 Nº Proposituras x Dias após Desastre
...
prop_dias_desastres <- df_ols %>% 
  filter(!is.na(dias_desastre_anterior)) %>% 
  group_by(dias_desastre_anterior) %>% 
  summarise(`Projetos Climáticos` = sum(n_proj_clima),
            `Total de Projetos` = sum(total_proj_dia)) %>% 
  pivot_longer(cols = c(`Projetos Climáticos`, `Total de Projetos`), names_to = "Origem", values_to = "Valor") %>% 
  ggplot(aes(x = dias_desastre_anterior, y = Valor, fill = Origem)) +
  geom_line()+
  geom_col()+
  ...
  xlim(0,2000)+
    theme(legend.position = "none",
        ...
        axis.text = element_text(size = 12),
        legend.text = element_text(size = 10))+
  facet_grid(rows = vars(Origem), 
             scales = "free_y")

prop_dias_desastres

ggsave(here("Figuras/2.5.1.1 Histograma proposituras desde última desastre.png"), prop_dias_desastres)

rm(prop_dias_desastres)
```

### 2.5.1.2 % Proposituras climáticas x Dias desde último desastre

```{r, warning = FALSE }

prop_clima_dias <- df_ols %>% 
  filter(!is.na(dias_desastre_anterior)) %>% 
  group_by(dias_desastre_anterior) %>% 
  summarise(`Projetos Climáticos` = sum(n_proj_clima),
            `Total de Projetos` = sum(total_proj_dia),
            `Agenda_Clima` = `Projetos Climáticos`/`Total de Projetos`) %>% 
  filter(!is.na(Agenda_Clima)) %>% 
  ggplot(aes(x = dias_desastre_anterior, y = `Agenda_Clima`)) +
  geom_line()+
  geom_col(fill = "darkgreen")+
  geom_vline(xintercept = 0)+
  geom_vline(xintercept = sum_dd[2], linetype = "dashed")+
  geom_vline(xintercept = sum_dd[3], linetype = "dashed")+
  geom_vline(xintercept = sum_dd[5], linetype = "dashed")+
  labs(title = "Percentual de proposituras climáticas desde o desastre mais recente no município", 
       y = "Percentual de Proposituras",
       x = "Dias desde o desastre mais recente",
       caption = "Elaborado pelo autor")+
  theme_bw()+
...
ggsave(here("Figuras/2.5.1.2 Percentual de proposituras climáticas desde última desastre.png"), prop_clima_dias, width = 10, height = 4)

rm(prop_clima_dias)
```

### 2.5.1.3 Densidade dos percentuais por dias levantados

```{r, warning= FALSE}

  
clima_dens_dias <- df_ols %>% 
  ggplot(aes(x = agenda_clima))+
  geom_density(fill = "darkgreen")+
  labs(title = "Densidade do percentual de agenda climática por dia", 
       y = "Frequência de Dias",
       x = "Percentual de proposituras climáticas (em log)",
       caption = "Elaborado pelo autor")+
  theme_bw()+
  theme(legend.position = "none",
        text = element_text(family = "Times New Roman"),
  ...
  scale_x_log10(labels = scales::label_percent(scale_cut = scales::cut_short_scale()))+
  scale_y_continuous(labels = label_percent())


clima_dens_dias

ggsave(here("Figuras/2.5.1.3 Densidade da distribuição dos percentuais climáticos diários.png"), clima_dens_dias, width = 10, height = 5)

rm(clima_dens_dias)
```

### 2.5.1.4 Densidade da agenda climática por tipo de desastre

```{r}
prop_dias_tipos_desastres <- df %>% 
  filter(!is.na(dias_desastre_anterior)) %>% 
  group_by(dias_desastre_anterior, desastre_anterior) %>% 
  summarise(`Projetos Climáticos` = n(),
            `Total de Projetos` = sum(total_tipo_proj_dia),
            `Agenda_Clima` = `Projetos Climáticos`/`Total de Projetos`) %>% 
  filter(!is.na(Agenda_Clima)) %>% 
  ggplot(aes(x = dias_desastre_anterior, y = Agenda_Clima, fill = desastre_anterior)) +
  geom_col()+
  geom_vline(xintercept = 0)+
...
ggsave(here("Figuras/2.5.1.4Densidade_proposituras_desastre.png"), prop_dias_tipos_desastres)

rm(prop_dias_tipos_desastres)

```

### 2.5.1.5 Tipo de Proposituras x Dias após Desastre
...
ggsave(here("Figuras/2.5.1.5Densidade_tipos_proposituras_dias_desastre.png"), tiproj_dias_desastres)

rm(tiproj_dias_desastres)
```

### 2.5.1.4 Dias após o Desastre por Tipo de Propositura e Categoria de Desastre

```{r, warning=FALSE}
library(tibble)
#Criando labels com contagem 
facet_counts <- df %>%
  mutate(Tipo = ifelse(Tipo %in% paleta_proposituras$Tipo, Tipo, "outros")) %>%
  filter(!is.na(desastre_anterior)) %>%
  count(desastre_anterior, Tipo, name = "n")

tipo_labels <- facet_counts %>%
  group_by(Tipo) %>%
  summarise(n = sum(n)) %>% 
  transmute(Tipo, label = paste0(Tipo, "\n(n = ", n, ")")) %>% 
  deframe()

desastre_labels <- facet_counts %>%
  group_by(desastre_anterior) %>%
  summarise(n = sum(n)) %>%
  transmute(desastre_anterior, label = paste0(desastre_anterior, "\n(n = ", n, ")")) %>%
  deframe()
...
tiproj_dias_desastres <- df %>% 
  ...
  scale_y_continuous(labels = label_percent())+
  facet_grid(
  desastre_anterior ~ Tipo,
  scales = "free_y",
  labeller = labeller(
    Tipo = as_labeller(tipo_labels),
    desastre_anterior = as_labeller(desastre_labels)))


tiproj_dias_desastres

ggsave(here("Figuras/2.5.1.4Densidade_tipos_proposituras_e_desastre.png"), tiproj_dias_desastres, width = 15, height = 15)

rm(tiproj_dias_desastres, facet_counts, tipo_labels, desastre_labels)
```

### 2.5.1.5 120 primeiros dias - Tipos de Proposituras
...
ggsave(here("Figuras/2.5.1.5Tipos_proposituras_120_dias_tipos_desastre.png"), tiproj_120_dias)
...
ggsave(here("Figuras/2.5.1.6Tipos_proposituras_120_dias_apos_desastre.png"), tiproj_deastres_120_dias)

rm(tiproj_deastres_120_dias)
```

### 2.5.2 Comparação com propostas no geral
...
ggsave(here("Figuras/2.5.2.1 Histograma TODAS proposituras desde último desastre.png"), prop_todos_dias_desastres)
...
ggsave(here("Figuras/2.5.2.2 Percentual das proposituras climáticas em relação ao total.png"), percent_todos_dias_desastres)

rm(percent_todos_dias_desastres)
```

### 2.5.3 Termos sobre clima x Dias após Desastre
...
dftermos <- df %>% 
  ...
  clima_termos %in% c(" pluvial", " pluviais", " chuva", " fortes chuvas",  " chuvoso") ~ "Chuvas",
  #Alagamentos e Inundações:
  clima_termos %in% c(" alagamento",  " inundação", " inundações", " enxurrada", " enchente") ~ "Inundações",
  #Movimentos de Massa:
  clima_termos %in% c(" deslizamento", " soterramento", " desabamento", " erosão") ~ "Movimentos de Massa",
  #Secas:
  clima_termos %in% c(" seca", " estiagem", " estiagens" ) ~ "Secas",
...
paleta_desastres <- paleta_desastres %>% 
  mutate(Tipo = ifelse(Tipo == "Alagamentos e Inundações", "Inundações", Tipo))
```
...
ggsave(here("Figuras/2.5.3.1 Densidade_termos_dias_desastre.png"), tiproj_dias_desastres)
...
ggsave(here("Figuras/2.5.3.2 Densidade_termos_e_desastre.png"), termos_dias_tipo_desastres, width = 12, height = 18)
...
ggsave(here("Figuras/2.5.3.3Termos_proposituras_120_dias_tipos_desastre.png"), termos_120_dias)
...
ggsave(here("Figuras/2.5.3.4Termos_120_dias_por_desastre.png"), termos_deastres_120_dias, width = 15, height = 20)

rm(termos_deastres_120_dias, sum_dd, dftermos)
```

### 2.5.4 % Proposituras Município x Risco Climático

```{r}
#Criando base com dados de df_risco
#Base 1
# Parte 1: Juntanto a base
dfrisco1 <- df_ols[
  , .(
      n_proj_clima = sum(n_proj_clima),
      total_proj = sum(total_proj_dia),
      agenda_clima = sum(n_proj_clima) / sum(total_proj_dia)
    ),
  by = .(
    Cod_IBGE_Mun,
...
# Parte 2: Convertendo para formato longo
dfrisco1 <- melt(
  dfrisco1,
  id.vars = c("Cod_IBGE_Mun", "n_proj_clima", "total_proj", "agenda_clima"),
  measure.vars = patterns("^AB2_"),
  variable.name = "Indicador",
  value.name = "Indice_Risco"
)

# Parte 3: Renomando variávelM
dfrisco1[, `:=`(
  Indice_Risco = as.numeric(Indice_Risco),
  parts = strsplit(as.character(Indicador), "_")
)]

dfrisco1[, Nivel := sapply(parts, function(x) x[1])]
dfrisco1[, Dimensao_Risco := sapply(parts, function(x) paste(x[2:3], collapse = " "))]
dfrisco1[, Risco_Climatico := sapply(parts, function(x) if (length(x) >= 4) paste(x[4:length(x)], collapse = " ") else NA_character_)]
dfrisco1[, parts := NULL]

# Part 4: Remove NA 
dfrisco1 <- dfrisco1[!is.na(Indice_Risco)]
```
...
prop_dimensao_risco <- dfrisco1 %>% 
  filter(Nivel == "AB2") %>% 
  ggplot(aes(y = agenda_clima, x = Indice_Risco))+
  geom_point()+
...
ggsave(here("Figuras/2.5.4.1.Proposituras_Dimensao_Risco.png"), prop_dimensao_risco, width = 10, height = 4)
...
risco_lms <- dfrisco1 %>%
  filter(Nivel == "AB2") %>% 
  group_split(Risco_Climatico) %>% #Agrupa por Indicador para regressões
  set_names(map_chr(., ~ unique(.x$Risco_Climatico))) %>% #Estabelece os nomes de cada regressão salva
  map(~ lm(agenda_clima ~ Indice_Risco, data = .x))  %>% #Roda todas as regressões em ação vetorizada
  map_dfr(tidy, .id = "Risco_Climatico") #Sumariza tabelas

kable(risco_lms, digits = 3)

rm(risco_lms, dfrisco1)
```
...
annot_df1 <- dfrisco2 %>%
  filter(Nivel > 2) %>%
  group_by(Dimensao_Risco, Risco_Climatico, Indice_Risco) %>%
  summarise(n = n(),
  ...
  group_split() %>%
  map_dfr(~ {
    model <- lm(Percentual ~ Indice_Risco, data = .x)
    coefs <- tidy(model)
    slope <- coefs %>% filter(term == "Indice_Risco")
    tibble(
      Dimensao_Risco = unique(.x$Dimensao_Risco),
      Risco_Climatico = unique(.x$Risco_Climatico),
      x = max(.x$Indice_Risco, na.rm = TRUE),
      y = max(.x$Percentual, na.rm = TRUE),
      label = paste0("β = ", round(slope$estimate, 3),
                     ", p = ", format.pval(slope$p.value, digits = 2, eps = 0.001))
    )
  }) %>% 
  mutate(Risco_Climatico = ifelse(Risco_Climatico == "Ameaça Climática", "Ameaça", Risco_Climatico))

#Gráfico
prop_indicador_risco <- dfrisco2 %>% 
  filter(Nivel > 2) %>% 
  group_by(Dimensao_Risco, Risco_Climatico, Indice_Risco) %>% 
  summarise(n = n(),
            Percentual = n/sum(total_tipo_proj_dia)) %>% 
  ...
  theme(legend.position = "bottom",
        ...
        legend.text = element_text(size = 10),
        strip.text = element_text(size = 12))+
  facet_grid(Risco_Climatico~Dimensao_Risco)

prop_indicador_risco 

ggsave(here("Figuras/2.5.4.2Proposituras_Indicador_Risco.png"), prop_indicador_risco, width = 10, height = 10)

rm(prop_indicador_risco, annot_df1)
```

```{r, warning = FALSE}
#RISCO CLIMÁTICO X TIPO DE PROPOSITURA


annot_df2 <- dfrisco2 %>%
  filter(Nivel == "2") %>%
  mutate(Tipo = ifelse(Tipo %in% paleta_proposituras$Tipo, Tipo, "outros")) %>%
  filter(!Tipo %in% c("proposições", "protocolo")) %>%
  group_by(Risco_Climatico, Indice_Risco, Tipo) %>%
  summarise(n = n(),
            Percentual = n / sum(total_tipo_proj_dia),
            .groups = "drop") %>%
  group_by(Risco_Climatico, Tipo) %>%
  group_split() %>%
  map_dfr(~ {
    model <- lm(Percentual ~ Indice_Risco, data = .x)
    coefs <- tidy(model)
...
prop_dimensao_risco <- dfrisco2 %>% 
  ...
  theme(legend.position = "none",
        ...
        legend.text = element_text(size = 10),
        strip.text = element_text(size = 10))+
  facet_grid(Risco_Climatico~Tipo)

prop_dimensao_risco

ggsave(here("Figuras/2.5.4.3.Proposituras_Dimensao_Risco_Tipo.png"), prop_dimensao_risco, width = 15, height = 10)

#rm(prop_dimensao_risco, annot_df2)
```
...
DesastresN <- readRDS(here("Dados/Desastres.RDS")) %>% 
  filter(Cod_IBGE_Mun %in% lista_municipios$Cod_IBGE_Mun) %>% 
  group_by(Cod_IBGE_Mun, Desastre) %>% 
  summarise(N_Desastre = n(), .groups = "drop") %>%
  pivot_wider(names_from = Desastre, values_from = N_Desastre) %>% 
  mutate(across(-Cod_IBGE_Mun, ~ replace_na(.x, 0)))

#Base

dfclimaXdesastres <- dfrisco2 %>%
  filter(Nivel == "2") %>%
  merge(DesastresN, by = c("Cod_IBGE_Mun"), all = T) %>%  
  pivot_longer(cols = c(`Estiagem e Seca`, `Incêndio Florestal`, `Vendavais e Ciclones`, "Alagamentos", "Enxurradas", "Inundações", "Erosão", `Chuvas Intensas`, "Granizo", `Movimento de Massa`), names_to = "Desastre", values_to = "N_Desastre") %>% 
  group_by(Cod_IBGE_Mun, Dimensao_Risco, Risco_Climatico, Indice_Risco, Desastre, N_Desastre) %>% 
  summarise(n = n(),
            Percentual = n/sum(total_tipo_proj_dia))
  
#Fazendo cálculos
annot_df3 <- dfclimaXdesastres %>% 
  group_by(Desastre, Risco_Climatico) %>% 
  group_split() %>%
  map_dfr(~ {
    model <- lm(N_Desastre ~ Indice_Risco, data = .x)
    coefs <- tidy(model)
    slope <- coefs %>% filter(term == "Indice_Risco")
    tibble(
      Risco_Climatico = unique(.x$Risco_Climatico),
      Desastre = unique(.x$Desastre),
      x = max(.x$Indice_Risco, na.rm = TRUE),
      y = max(.x$N_Desastre, na.rm = TRUE),
      label = paste0("β = ", round(slope$estimate, 3),
                     ", p = ", format.pval(slope$p.value, digits = 2, eps = 0.001))
    )
  })

climaXdesastres <- dfclimaXdesastres %>% 
  ggplot(aes(x = Indice_Risco, y = N_Desastre, color = Desastre)) +
  geom_point()+
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")+
  ...
  theme(legend.position = "none",
        ...
        legend.text = element_text(size = 10),
        strip.text = element_text(size = 10))+
  facet_grid(Desastre~Risco_Climatico)

climaXdesastres

ggsave(here("Figuras/2.5.4.4.Desastres_Indicador_Risco.png"), climaXdesastres, width = 10, height = 12)

rm(climaXdesastres, annot_df3, dfclimaXdesastres, coefs, DesastresN)
```

```{r}

#Óbitos em Desastres x Níveis de Risco
DesastresO <- readRDS(here("Dados/Desastres.RDS")) %>% 
  filter(Cod_IBGE_Mun %in% lista_municipios$Cod_IBGE_Mun) %>% 
  group_by(Cod_IBGE_Mun, Desastre) %>% 
  summarise(obitos = sum(DH_MORTOS), .groups = "drop") %>%
  pivot_wider(names_from = Desastre, values_from = obitos) %>% 
  mutate(across(-Cod_IBGE_Mun, ~ replace_na(.x, 0)))

dfclimaXobitos <- dfrisco2 %>%
  filter(Nivel == "2") %>%
  merge(DesastresO, by = c("Cod_IBGE_Mun"), all = T) %>%  
  pivot_longer(cols = c(`Estiagem e Seca`, `Incêndio Florestal`, `Vendavais e Ciclones`, "Alagamentos", "Enxurradas", "Inundações", "Erosão", `Chuvas Intensas`, "Granizo", `Movimento de Massa`), names_to = "Desastre", values_to = "Obitos") %>% 
  group_by(Cod_IBGE_Mun, Dimensao_Risco, Risco_Climatico, Indice_Risco, Desastre, Obitos) %>% 
  summarise(n = n(),
            Percentual = n/sum(total_tipo_proj_dia)) %>% 
  filter(Obitos > 0)

#Cálculos
annot_df4 <- dfclimaXobitos %>% 
  group_by(Risco_Climatico) %>% 
  group_split() %>%
  map_dfr(~ {
    model <- lm(Obitos ~ Indice_Risco, data = .x)
    coefs <- tidy(model)
    slope <- coefs %>% filter(term == "Indice_Risco")
    tibble(
      Risco_Climatico = unique(.x$Risco_Climatico),
      x = max(.x$Indice_Risco, na.rm = TRUE),
      y = max(.x$Obitos, na.rm = TRUE),
      label = paste0("β = ", round(slope$estimate, 3),
                     ", p = ", format.pval(slope$p.value, digits = 2, eps = 0.001))
    )
  })
...
#Gráfico
climaXobitos <- dfclimaXobitos %>% 
  ggplot(aes(x = Indice_Risco, y = Obitos)) +
  geom_point(aes(color = Desastre))+
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth")+
  geom_text(
    ...
    size = 3.5,
    family = "Times New Roman")+
  labs(title = "Número de Mortes por Desastres x Índice de Risco", 
       x = "Índice de Risco",
       y = "Número de Óbitos",
       caption = "Elaborado pelo autor")+
  scale_color_manual(values = paleta_desastres$Cores, limits = paleta_desastres$Tipo)+
  ...
  theme(legend.position = "none",
        ...
        legend.text = element_text(size = 10),
        strip.text = element_text(size = 12))+
  facet_wrap(~Risco_Climatico)


climaXobitos

ggsave(here("Figuras/2.5.4.5.Obitos_Indicador_Risco.png"), climaXobitos, width = 8, height = 8)

rm(climaXobitos, DesastresO, dfclimaXobitos, dfrisco2, annot_df4)
```

### 2.5.5 % Proposituras Município x Nº de OSCs

```{r}
df_osc <- df_ols %>% 
  filter(Ano == 2023) %>% 
  group_by(Cod_IBGE_Mun) %>% 
  summarize(n = sum(n_proj_clima),
            total = sum(total_proj_dia),
            n_OSCs = max(n_OSCs),
            agenda_clima = n/total)
...
annot_df5 <- df_osc %>% 
  group_split() %>%
  map_dfr(~ {
    model <- lm(agenda_clima ~ n_OSCs, data = .x)
    coefs <- tidy(model)
    slope <- coefs %>% filter(term == "n_OSCs")
    tibble(
      x = max(.x$n_OSCs, na.rm = TRUE),
      y = max(.x$agenda_clima, na.rm = TRUE),
      label = paste0("β = ", round(slope$estimate, 3),
                     ", p = ", format.pval(slope$p.value, digits = 2, eps = 0.001))
    )
  })

proposituras_oscs <-  df_osc %>% 
  ggplot(aes(x = n_OSCs, y = agenda_clima)) +
  geom_point()+
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth",
...
ggsave(here("Figuras/2.5.5.1.Proposituras_OSCs.png"),proposituras_oscs, width = 5, height = 5)

rm(proposituras_oscs, annot_df5, df_osc)
```
...
ggsave(here("Figuras/2.5.5.2.Tipos_Proposituras_OSCs.png"),tipos_proposituras_oscs, width = 10, height = 5)

rm(tipos_proposituras_oscs)
```

### 2.5.6 %Proposituras climáticas x Orçamento municipal

```{r, warning = FALSE}
df_orcamento <- df_ols %>% 
  filter(Ano == 2024) %>% 
  group_by(Cod_IBGE_Mun, Receitas_pCap) %>% 
  summarize(n = sum(n_proj_clima),
            total = sum(total_proj_dia),
            agenda_clima = n/total) %>% 
...
annot_df6 <- df_orcamento %>% 
  ungroup() %>% 
  group_split() %>%
  map_dfr(~ {
    model <- lm(agenda_clima ~ Receitas_pCap, data = .x)
    coefs <- tidy(model)
    slope <- coefs %>% filter(term == "Receitas_pCap")
    tibble(
      x = max(.x$Receitas_pCap, na.rm = TRUE),
      y = max(.x$agenda_clima, na.rm = TRUE),
      label = paste0("β = ", round(slope$estimate, 3),
                     ", p = ", format.pval(slope$p.value, digits = 2, eps = 0.001))
    )
...
proposituras_orcamento <-  df_orcamento %>% 
  ggplot(aes(x = Receitas_pCap, y = agenda_clima)) +
  geom_point()+
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth",
  ...
  geom_text(
    ...
    size = 3.5,
    family = "Times New Roman")+
  labs(title = "Percentual de proposituras apresentadas x Receitas Totais do município em 2024, per capita", 
       x = "Receitas Totais do Município, per capita (em R$)",
       y = "Percentual de proposituras no ano",
       caption = "Elaborado pelo autor")+
  theme_bw()+
  scale_y_continuous(labels = label_percent(accuracy = 1)) +
...
ggsave(here("Figuras/2.5.6.1.Proposituras_Orcamento.png"),proposituras_orcamento, width = 5, height = 5)

rm(proposituras_orcamento, annot_df6, df_orcamento)
```
...
ggsave(here("Figuras/2.5.6.2.Tipos_proposituras_Orcamento.png"),tipos_proposituras_orcamento)

rm(tipos_proposituras_orcamento)
```

### 2.5.7 Nº de Proposituras x Dias para Eleição
...
ggsave(here("Figuras/2.5.7.1 Histograma proposituras até próxima eleição.png"), prop_dias_eleicao, width = 10, height = 5)
...
ggsave(here("Figuras/2.5.7.2Densidade proposituras até próxima eleição.png"), prop_densi_eleicao)
...
p_agenda_eleicao <- df_ols %>% 
  ...
  mutate(dias_proxima_eleicao = -1 * dias_proxima_eleicao) %>%
  mutate(bin_30d = floor(dias_proxima_eleicao / 30) * 30) %>% 
  group_by(bin_30d) %>%
  summarise(
    agenda_clima = mean(agenda_clima, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  ggplot(aes(x = bin_30d, y = agenda_clima, fill = T)) +
  geom_col(width = 35)+ 
  geom_vline(xintercept = 0)+
  geom_vline(xintercept = -365, linetype = "dashed")+
  geom_vline(xintercept = -730, linetype = "dashed")+
  geom_vline(xintercept = -1095, linetype = "dashed")+
  labs(title = "Percentual de proposituras climáticas em relação aos dias até a próxima eleição", 
       y = "Percentual de Proposituras",
       x = "Dias até a próxima eleição",
       caption = "Elaborado pelo autor")+
  theme_bw()+
...
ggsave(here("Figuras/2.5.7.3  Percentual de proposituras por dia até próxima eleição.png"), p_agenda_eleicao, width = 10, height=5)
...
rm(paletas, paleta_climatermos, paleta_desastres, paleta_proposituras, paleta_regiao, df, df_ols, lista_municipios, risco_lms)
```
