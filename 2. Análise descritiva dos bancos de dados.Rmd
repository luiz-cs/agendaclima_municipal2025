...
ggsave(here("Figuras/2.4.1.2.2 Densidade_tipos_proposituras_e_desastre.png"), tiproj_dias_desastres, width = 15, height = 30)
...
ggsave(here("Figuras/2.4.1.3.2 Densidade_termos_e_desastre.png"), termos_dias_tipo_desastres, width = 15, height = 20)
...
ggsave(here("Figuras/2.4.2.1.Proposituras_Dimensao_Risco.png"), prop_dimensao_risco)
...
### FAZER RISCO CLIMÁTICO X N DESASTRES
...
ggsave(here("Figuras/2.4.2.4.Obitos_Indicador_Risco.png"), climaXobitos, width = 15, height = 20)
...
proposituras_orcamento <- df %>% 
  ...
  stat_smooth(method = "lm",
              ...
              geom = "smooth",
              aes(color = NA))+
  labs(title = "Percentual de proposituras apresentadas x Receita dos municípios em 2023", 
       x = "Receita do governo municipal em 2023",
       y = "Percentual de proposituras no ano")+
  theme_bw()+
  scale_y_continuous(labels = label_percent(accuracy = 1)) +
...
desastres_caso %>% 
  ggplot(aes(y = DH_total_danos_humanos, x = Data_Evento, fill = Desastre))+
  geom_col()+
  theme_bw()+
  theme(legend.position = "bottom")+
  scale_fill_manual(values = paleta_desastres$Cores, limits = paleta_desastres$Tipo)+
