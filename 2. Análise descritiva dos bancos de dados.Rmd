...
ggsave(here("Figuras/2.4.1.3.2 Densidade_termos_e_desastre.png"), termos_dias_tipo_desastres, width = 15, height = 30)
...
rm(termos_deastres_120_dias, sum_dd)
```
...
r1 <- Risco %>% 
  filter(Indicador == "4_Desastres_Hidrológicos_Sensibilidade") %>% 
  group_by(Indicador, Indice_Risco) %>% 
  summarise(n = n(),
            Percentual = n/sum(total_tipo_proj_dia)) 

l1 <- lm(Percentual ~ Indice_Risco, r1)
